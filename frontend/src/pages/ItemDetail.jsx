...
const mockItems = {
   1: { id: 1, name: 'iPhone 13 Pro', type: 'lost', category: 'Elektronik', location: 'Kadikoy, Istanbul', date: '2026-05-20', description: 'Siyah renkli, özel yapım bir deri kılıfı var. İçinde önemli belgelerim var. Bulan kişinin ulaştırması çok önemli.', image: 'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=800&h=400&fit=crop', user: 'Ahmet Yılmaz', createdAt: '2026-05-21', status: 'active' },
   2: { id: 2, name: 'Deri Cüzdan', type: 'found', category: 'Cüzdan & Çanta', location: 'Besiktas Sahil', date: '2026-05-22', description: 'İçinde kimlik ve kartlar bulunuyor. Beşiktaş sahilinde yürüme yolunda bankların birinde buldum.', image: 'https://images.unsplash.com/photo-1627123424574-724758594e93?w=800&h=400&fit=crop', user: 'Mert Gur', createdAt: '2026-05-22', status: 'active' },
};

export default function ItemDetail() {
   const { id } = useParams();
   const item = mockItems[id] || mockItems[1]; // Fallback if not found in mock
            ...
            <div className="p-6 md:p-8">
               <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-6">
                  <div>
                     ...
                     <p className="text-gray-500 flex items-center">
                        Reported by <span className="font-semibold text-gray-700 ml-1">{item.user}</span>
                  ...
                  <div className="flex items-center gap-3">
                     ...
                     <button className="flex-1 md:flex-none flex items-center justify-center px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors">
                        <MessageSquare className="mr-2 h-5 w-5" /> Contact {item.type === 'lost' ? 'Owner' : 'Finder'}
                     </button>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-6 border-y border-gray-100">
                  <div className="col-span-2 space-y-6">
                     <div>
                        <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center">
                           <Info className="mr-2 h-5 w-5 text-indigo-500" /> Description
                        </h3>
                  ...
                  <div className="space-y-4 bg-gray-50 p-6 rounded-xl border border-gray-100">
                     <h3 className="font-bold text-gray-900 border-b border-gray-200 pb-2">Details</h3>
                     ...
                     <div className="space-y-4">
                        ...
                        <div>
                           <span className="text-sm font-medium text-gray-500 block mb-1">Date {item.type === 'lost' ? 'Lost' : 'Found'}</span>
                     ...
                     <div className="mt-6 pt-4 border-t border-gray-200">
                        <button className="flex items-center text-sm font-medium text-red-600 hover:text-red-800 transition-colors">
                           <AlertTriangle className="mr-2 h-4 w-4" /> Report False Listing
                        </button>
                     </div>
                  </div>
               </div>
